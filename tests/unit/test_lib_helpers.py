...
def test_get_upgrade_path():
    assert get_upgrade_path(
        "9.1.6", "10.1.9", UPGRADE_PATHS["PANOS_FIRMWARE_LIST_OFFICIAL"]
    ...
    assert get_upgrade_path("9.1.6", "10.1.9", UPGRADE_PATHS["PANOS_FIRMWARE_LIST_MAJOR_ONLY"]) == [
        "10.0.0",
        "10.1.0",
        "10.1.9",
    ]
...
def test_get_upgrade_path_current_greater_than_target():
    with pytest.raises(ValueError, match="Target version must be newer than current version."):
        get_upgrade_path("10.1.9", "9.1.6", UPGRADE_PATHS["PANOS_FIRMWARE_LIST_MAJOR_ONLY"])
...
def test_get_upgrade_path_current_equals_target():
    with pytest.raises(ValueError, match="Target version equals current version. No upgrade necessary."):
        get_upgrade_path("10.1.9", "10.1.9", UPGRADE_PATHS["PANOS_FIRMWARE_LIST_MAJOR_ONLY"])
